    ...
    fetch(ORIGIN + '/' + MANIFEST_URL)
      ...
      .then((content) => {
        ...
        content = content.replace(
          new RegExp(
            '(NETWORK|FALLBACK):' + '((?!(NETWORK|FALLBACK|CACHE):)[\\w\\W]*)',
            'gi'
          ),
          ''
        );
...
function check_manifest() {
  ...
  caches
    .match(new Request(ORIGIN + '/' + MANIFEST_URL))
    .then((response) => {
    ...
    .then((content) => {
      if (content != cacheManifest) {
        console.log(
          'マニュフェストファイルに変更があった為、リソースを再キャッシュします'
        );
        //現在のキャッシュをクリア
    ...
    .then(() => {
      return load_manifest().then((list) => download_files(list));
    })
    .catch((err) => {
